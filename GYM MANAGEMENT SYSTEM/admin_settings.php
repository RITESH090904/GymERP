...
$profileStmt = $conn->prepare("SELECT id, name, phone, email FROM users WHERE id = ? AND role = 'admin' LIMIT 1");
$profileStmt->bind_param("i", $adminId);
$profileStmt->execute();
$profileStmt->store_result();
$profileStmt->bind_result($userId, $userName, $userPhone, $userEmail);
$profileStmt->fetch();
$profileStmt->close();
...
if (isset($_POST['save_profile'])) {
    ...
    if ($check->num_rows > 0) {
        ...
        $stmt->bind_param("sssi", $name, $phone, $email, $adminId);
        $stmt->execute();
        $stmt->close();
        $_SESSION['admin'] = $name;
        $userName = $name;
...
if (isset($_POST['change_password'])) {
    ...
    if (!$passwordHash || !password_verify($currentPassword, $passwordHash)) {
        ...
        $updatePassword->bind_param("si", $newHash, $adminId);
...
body{
    ...
    url('img/gym.jpg') no-repeat center center;
